...
text_roundtrip(Term, Peek, Code, Mode, Alias) :-
    open('/tmp/webentail-iso-text.txt', write, Output, [alias(iso_text_output), type(text)]),
    writeq(iso_text_output, sample(42)),
    put_char(iso_text_output, '.'),
    put_char(iso_text_output, 'Z'),
    close(Output),
    open('/tmp/webentail-iso-text.txt', read, Input, [alias(iso_text_input), eof_action(eof_code)]),
...
binary_roundtrip(Peek, Byte, End) :-
    open('/tmp/webentail-iso-binary.bin', write, Output, [type(binary)]),
    put_byte(Output, 65),
    close(Output),
    open('/tmp/webentail-iso-binary.bin', read, Input, [type(binary), eof_action(eof_code)]),
...
read_term_metadata(ok) :-
    open('/tmp/webentail-iso-read-term.txt', write, Output, []),
    writeq(Output, pair(X, X, Y)),
    put_char(Output, '.'),
    close(Output),
    open('/tmp/webentail-iso-read-term.txt', read, Input, [reposition(true)]),
