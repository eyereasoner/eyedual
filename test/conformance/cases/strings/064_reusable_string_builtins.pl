...
answer(trim, X) :- trim("  Hello WebEntail  ", X).
answer(lower, X) :- lowercase("Hello WebEntail", X).
answer(upper, X) :- uppercase("Hello WebEntail", X).
...
answer(atom_string, X) :- atom_string(webentail, X).
