...
answer(trim, X) :- trim("  Hello EyeDual  ", X).
answer(lower, X) :- lowercase("Hello EyeDual", X).
answer(upper, X) :- uppercase("Hello EyeDual", X).
...
answer(atom_string, X) :- atom_string(eyedual, X).
