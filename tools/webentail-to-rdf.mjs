...
import { webEntailQuadToNQuad } from './rdf-codec.mjs';

export function extractWebEntailRdf(source) {
  ...
  for (const c of program.clauses) {
    ...
    const line = webEntailQuadToNQuad(c.head);
...
async function main(argv) {
  ...
  const result = extractWebEntailRdf(source);
  if (output === '-') process.stdout.write(result); else await fs.writeFile(output, result);
}
function usage() { process.stdout.write('Usage: node tools/webentail-to-rdf.mjs [options] [input.pl|-]\n\n  -o, --output FILE  Write N-Quads to FILE\n'); }
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) main(process.argv.slice(2)).catch((e) => { process.stderr.write(`webentail-to-rdf: ${e.message}\n`); process.exitCode = 1; });
