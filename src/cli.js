// Command-line interface for WebEntail.
...
export async function main(argv) {
  ...
  if (options.version) {
    process.stdout.write(`webentail ${await packageVersion()}\n`);
    return;
  }
...
async function runDefault(engine, program, options) {
  ...
  const lines = new Set();
  const registry = engine.getWebEntailRegistry();
...
async function usage(stream) {
  stream.write(`webentail ${await packageVersion()}

Usage:
  webentail [options] [file-or-url.pl|- ...]

Input:
  file-or-url.pl        Read an WebEntail program from a local file or http(s) URL.
  -                     Read an WebEntail program from standard input.
...
function printWarnings(program) {
  ...
  process.stderr.write('webentail warning: unstratified negation\n');
...
function printStats(stats) {
  process.stderr.write('webentail stats:\n');
