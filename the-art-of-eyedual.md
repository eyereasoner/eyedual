...
EyeDual turns facts and rules into answers and inspectable proofs. This book is an
...
This book is also the reference for the EyeDual implementation. EyeDual is a
...
program with EyeDual means learning to move comfortably between these views.

The name *EyeDual* reflects the two standards at its foundation: ISO Prolog for
executable rules and W3C RDF for linked data. EyeDual implements a broad ISO
...
capabilities around that standards-based foundation. EyeDual does not attempt to
...
across tools. EyeDual aims to provide a compact implementation of those standards
with explanations and practical host integration, not another proprietary rule
language.

This places EyeDual in a tradition that joins automated deduction, database
...
deductive databases emphasized finite relations and fixed points. EyeDual
...
EyeDual's focused surface makes it unusually easy to see in running examples.

Complete EyeDual code displays from the book are also available as files under
[`examples/book/`](https://github.com/eyereasoner/eyedual/tree/main/examples/book/), grouped by chapter. From a source checkout,
use Node.js 18 or newer, install the dependencies, and run the CLI:

```sh
npm install
node bin/eyedual.js examples/socrates.pl
```

The EyeDual command should print:
...
[browser playground](https://eyereasoner.github.io/eyedual/playground). Paste
...
EyeDual library by default, so relations such as `append/3` and `member/2` need no
...
- an `eyedual` block is Prolog source accepted by EyeDual; complete blocks are extracted under
...
- a `text` block shows output, a trace, a data shape, or pseudocode and is not
  necessarily accepted as EyeDual input;
- a `sh` or `js` block is a host command or embedding example.

Top-level programs under [`examples/`](https://github.com/eyereasoner/eyedual/tree/main/examples/) are the complete runnable
...
17 to see where EyeDual's hybrid execution and proof-oriented design differ.
...
- [16. Embedding EyeDual](#16-embedding-eyedual)
...
- [40. Running EyeDual: command line and corpus](#40-running-eyedual-command-line-and-corpus)
...
A **query declaration** selects the relation whose ground answers EyeDual prints:

```eyedual
child(Child, Parent) :- parent(Parent, Child).
eyedual --goal 'child(X, Y)' program.pl
```
...
EyeDual distinguishes solutions found by the solver from answers printed by the
CLI. A query such as `eyedual --goal 'parent(X, Y)' program.pl` can find the three source facts
...
`eyedual --goal 'child(X, Y)' program.pl` ask for, and (3) predict which output changes after adding
`parent(diego, elena).`

## 2. Terms, variables, and substitution

Prolog programs accepted by EyeDual are built from terms:
...
EyeDual exposes unification as `=/2`:

```eyedual
same_shape(Pair) :- (Pair = pair(X, X)).

eyedual --goal 'same_shape(pair(red, red))' program.pl
eyedual --goal 'same_shape(pair(red, blue))' program.pl
```
...
**Checkpoint.** Without running EyeDual, decide whether each pair unifies:
...
read as procedures. EyeDual begins further downstream: it offers a compact
...
goals. EyeDual normally selects from left to right. As a safe optimization, it
...
least-Herbrand-model account. EyeDual sits downstream of this sequence:
...
Herbrand: ground terms and instances as a proof-theoretic foundation
  ...
  -> EyeDual: a focused Prolog implementation with inspectable derivations
```
...
question: what can a term denote? EyeDual uses **Herbrand semantics**. Its
...
The runnable
[`examples/herbrand-semantics.pl`](https://github.com/eyereasoner/eyedual/blob/main/examples/herbrand-semantics.pl) example and
...
logic-programming sense: EyeDual searches for substitutions that make the goal
follow from the program.

EyeDual has no blank nodes or existential variables in rule heads. When a rule
...
identical. EyeDual performs an occurs check whenever unification would bind a
...
EyeDual's evaluator is goal-directed. It resolves selected goals against facts,
...
ancestor(X, Z) :- parent(X, Y), ancestor(Y, Z).
eyedual --goal 'ancestor(X, Y)' program.pl
```
...
forever. EyeDual analyzes predicate dependencies and automatically tables
...
No mutation occurs; every call receives a new term. EyeDual also includes
...
eyedual --goal 'answer(X)' program.pl
```
...
The visited list makes a finite state space explicit. EyeDual is strongest when
...
numerical kernels generally belong in a host, with EyeDual as the decision layer.
...
These distinctions explain EyeDual's conservative treatment. Negation and
...
EyeDual goals are supplied by the host, for example
`eyedual --goal 'child(X, Y)' program.pl`. EyeDual prints ground answers, removes
...
Proof output is valid EyeDual input:

```sh
eyedual --proof examples/socrates.pl > socrates.why.pl
```
...
```sh
node bin/eyedual.js examples/integrity-check.pl
```
...
```sh
eyedual --stats examples/observability-log-correlation.pl
```
...
EyeDual keeps RDF 1.2 at an explicit standards boundary. Adapter tools translate
...
nested data does not assert that nested triple as a global EyeDual fact.

## 16. Embedding EyeDual
...
```js
const source = `
eyedual --goal 'path(a, X)' program.pl
edge(a, b).
edge(b, c).
path(X, Y) :- edge(X, Y).
...
the complete EyeDual library predicates as native JavaScript
...
An embedder can start from the default EyeDual registry and add a host relation. A
...
import {
  atom,
  createEyeDualRegistry,
  run,
  unify
} from 'eyedual';

const registry = createEyeDualRegistry();
...
Treat remote source as executable logic. Although EyeDual has no arbitrary host
...
a document extractor, or an agent. An **EyeDual Socket** gives that opening a
name and a contract:
...
socket states what kind of knowledge may enter; EyeDual checks and combines the
...
what the host validates before constructing terms, what EyeDual derives from
...
calls and accumulates answers. EyeDual's automatic positive tabling is smaller
...
EYE belongs to that proof-producing Semantic Web tradition. EyeDual adopts the
...
<figure>
  <img src="book-assets/part-4-craft.svg" alt="A logic programmer works between domain sketches, design questions, and tested EyeDual clauses.">
...
EyeDual removes several classic Prolog control devices, especially cut. The
...
EyeDual now supplies the Part 1 control, dynamic-database, operator, and I/O
...
eyedual --goal 'ancestor(ada, Who)' program.pl
```
...
eyedual --goal 'complete_sentence([the, robot, helps, a, scientist])' program.pl
```
...
EyeDual relation that inspects it, and the environment or state used to interpret
...
eyedual --goal 'assignment(Worker, Task)' program.pl
```
...
eyedual --goal 'permit(Person, Zone)' program.pl
eyedual --goal 'reason(Person, Zone, Reason)' program.pl
```
...
Authenticate source systems in the host, convert records to EyeDual facts, run
...
unfolding, folding, and specialization. The inheritance for EyeDual is not a
...
craft. Part V pays tribute to that breadth through EyeDual's explicit subset:
...
EyeDual is a very small descendant of those questions. It is not a foundation
...
| Mathematical act | EyeDual form | Operational consequence |
| --- | --- | --- |
...
EyeDual execution answer different questions unless the encoding between them is
...
eyedual --goal 'triple(A, B, C)' program.pl
```
...
exists without furnishing an efficient construction. An EyeDual query produces
...
eyedual --goal 'integer_rectangle(24, W, H)' program.pl
```
...
whether EyeDual regards them as structurally equal, whether the domain regards
...
eyedual --goal 'counterexample_to_odd_square(N)' program.pl
```
...
operations and relations. EyeDual can enumerate candidates, apply axioms as
...
Every trustworthy EyeDual conclusion has the same broad shape:
...
EyeDual has smaller, immediate limits:
...
Before trusting an EyeDual conclusion, ask:
...
5. Write a one-page “trust contract” for an embedded EyeDual service: accepted
...
provability, decidability, and computation. EyeDual's finite bounds, mode
...
eyedual --goal 'path(a, b)' program.pl
eyedual --goal 'path(a, c)' program.pl
```
...
eyedual --goal 'expected_absence' program.pl
```
...
```eyedual
eyedual --goal 'append([a, b], [c], Whole)' program.pl
eyedual --goal 'append(Prefix, Suffix, [a, b])' program.pl
```
...
eyedual --goal 'bounded_double_law' program.pl
```
...
```eyedual
eyedual --goal 'eligible(alex)' program.pl
```
...
eyedual --goal 'candidate_debug(Person, Age)' program.pl
eyedual --goal 'adult_debug(Person, Age)' program.pl
```
...
eyedual --goal 'disagreement(N, S)' program.pl
```
...
Integer arithmetic has similarly precise choices. In EyeDual's supported
...
- [`iso-control-and-errors.pl`](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-control-and-errors.pl)
  covers `call/1`, `once/1`, cut, if-then-else, and recovery;
- [`iso-grouped-solutions.pl`](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-grouped-solutions.pl)
  contrasts the three collectors and inspects a source clause; and
- [`iso-integer-arithmetic.pl`](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-integer-arithmetic.pl)
...
These are atom relations, not the EyeDual library's string convenience
predicates. Quoted atoms such as `'λ'` remain atoms; double-quoted values remain
EyeDual strings.

[`iso-reflective-terms.pl`](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-reflective-terms.pl)
walks through shape, rebuilding, fresh copying, variables, and order.
[`iso-atomic-conversion.pl`](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-atomic-conversion.pl)
...
Run [`iso-dynamic-database.pl`](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-dynamic-database.pl)
for an explicitly stateful queue and
[`iso-operators.pl`](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-operators.pl)
...
[`iso-term-io.pl`](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-term-io.pl)
...
The supported ISO facilities make EyeDual suitable for more than closed rule
files:
...
Corrigenda 1:2007, 2:2012, and 3:2017. EyeDual implements the compatibility
profile documented here; it does not claim certification as a complete
conforming processor.

Prolog source accepted by EyeDual is UTF-8. `%` starts a line comment and
...
describe the grammar; they are not characters written in EyeDual source.
...
EyeDual supports cut, operator declarations, dynamic database updates, grouped
...
Supported output syntax is designed to be readable as Prolog input accepted by EyeDual.
...
EyeDual's default registry contains the built-ins in its ISO compatibility
profile. Where a predicate is defined by ISO/IEC 13211-1:1995, EyeDual uses its
...
EyeDual's documented profile order is not the complete ISO term order: strings
...
The runtime registry combines the supported ISO Prolog profile with the EyeDual library, implemented entirely in JavaScript. It is
...
```sh
eyedual program.pl
eyedual -p program.pl        # add proof output
```
...
Most EyeDual library predicates are projections or filters. When an input is
...
| `local_time(-Date)` | Produces the host-local calendar date as `"YYYY-MM-DD"`. Tests and reproducible hosts may set `EYEDUAL_LOCAL_TIME`. |
...
eyedual --goal 'answer(Kind, Value)' program.pl
```
...
These relations are JavaScript implementations provided by the EyeDual library.
...
| `min_list(+List,-Min)`, `max_list(+List,-Max)` | Select by EyeDual term order, not numeric coercion. Empty lists fail. |
...
number. Most string predicates accept any of those inputs but produce EyeDual
...
| `term_string(+Term,-Text)` | Renders a nonvariable term using EyeDual readback syntax. It does not parse text back into a term. |
...
ISO `findall/3` is present in both registries. The EyeDual library aggregates follow
...
## 40. Running EyeDual: command line and corpus
...
<figure>
  <img src="book-assets/cli-observation-loop.svg" alt="An EyeDual source and query enter the CLI, which separates ground answers and proofs on standard output, warnings and statistics on standard error, and a process status for automation; comparison leads back to program revision.">
...
```sh
eyedual --goal 'ancestor(ada, Who)' examples/ancestor.pl
```
...
`eyedual` prints help. When options are present but no input is named, stdin is
...
```sh
eyedual --goal 'ancestor(X, Y)' examples/ancestor.pl
eyedual --proof --goal 'type(X, Y)' examples/socrates.pl
eyedual --warnings --goal 'answer(X)' test/conformance/warnings/negation/unstratified_mutual.pl
eyedual --stats --goal 'path(a, X)' examples/path-discovery.pl > answers.pl 2> run.stats
```

Normal answers and `why/2` terms go to stdout, which makes them suitable for a
golden file or another EyeDual input. Warnings and statistics go to stderr so
...
<figure>
  <img src="book-assets/example-landscape.svg" alt="A map connects EyeDual examples across mathematics, search, planning, policy, science, program analysis, symbolic systems, and RDF.">
...
explanation under [examples/proof](https://github.com/eyereasoner/eyedual/tree/main/examples/proof/). The thematic tables below link every top-level program and open the program
itself rather than merely naming it.

The generated [`examples/book/`](https://github.com/eyereasoner/eyedual/tree/main/examples/book/) tree serves a different
purpose: it mirrors the complete inline EyeDual displays chapter by chapter.
...
node bin/eyedual.js --proof examples/ancestor.pl
```
...
| --- | --- | --- |
| [Combinatorics Findall Sort](https://github.com/eyereasoner/eyedual/blob/main/examples/combinatorics-findall-sort.pl) | Eyelet-inspired combinations example using findall/3 and sort/2. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/combinatorics-findall-sort.pl) |
| [Floating Point](https://github.com/eyereasoner/eyedual/blob/main/examples/floating-point.pl) | Floating-point arithmetic and comparisons. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/floating-point.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/floating-point.pl) |
| [Atomic conversion](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-atomic-conversion.pl) | Atom splitting, character atoms, Unicode codes, and numeric parsing. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/iso-atomic-conversion.pl) |
| [Control and errors](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-control-and-errors.pl) | `call/1`, `once/1`, cut, if-then-else, `throw/1`, and `catch/3`. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/iso-control-and-errors.pl) |
| [Dynamic database](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-dynamic-database.pl) | Initialization and ordered updates to a declared dynamic procedure. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/iso-dynamic-database.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/iso-dynamic-database.pl) |
| [Grouped solutions](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-grouped-solutions.pl) | `findall/3`, `bagof/3`, `setof/3`, existential qualification, and `clause/2`. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/iso-grouped-solutions.pl) |
| [Integer arithmetic](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-integer-arithmetic.pl) | Integer quotient/remainder choices plus bit operations. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/iso-integer-arithmetic.pl) |
| [Operators](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-operators.pl) | Custom syntax, standard term order, and operator-table inspection. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/iso-operators.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/iso-operators.pl) |
| [Reflective terms](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-reflective-terms.pl) | Term shape, construction, copying, variables, identity, and standard order. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/iso-reflective-terms.pl) |
| [Term I/O](https://github.com/eyereasoner/eyedual/blob/main/examples/iso-term-io.pl) | Text-stream lifecycle, canonical writing, reading, metadata, and end state. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/iso-term-io.pl) |
| [Term Tools](https://github.com/eyereasoner/eyedual/blob/main/examples/term-tools.pl) | Term-tool builtins for inspecting, constructing, rendering, and validating structured terms. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/term-tools.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/term-tools.pl) |
...
| --- | --- | --- |
| [Age](https://github.com/eyereasoner/eyedual/blob/main/examples/age.pl) | Arithmetic comparison acts as a filter after a fact supplies the age. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/age.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/age.pl) |
| [Ancestor](https://github.com/eyereasoner/eyedual/blob/main/examples/ancestor.pl) | The canonical base-plus-recursive definition computes a transitive family relation. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/ancestor.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/ancestor.pl) |
| [Animal](https://github.com/eyereasoner/eyedual/blob/main/examples/animal.pl) | Several clauses form a small classification theory with inspectable reasons. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/animal.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/animal.pl) |
...
| [Backward](https://github.com/eyereasoner/eyedual/blob/main/examples/backward.pl) | A tiny derived fact justified by a numeric comparison in an ordinary Horn rule. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/backward.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/backward.pl) |
| [Cat Koko](https://github.com/eyereasoner/eyedual/blob/main/examples/cat-koko.pl) | Named Skolem-style witnesses standing in for the existential witnesses of the original N3 example. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/cat-koko.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/cat-koko.pl) |
| [Derived rule](https://github.com/eyereasoner/eyedual/blob/main/examples/derived-rule.pl) | A conclusion depends on another derived predicate rather than directly on a source fact. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/derived-rule.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/derived-rule.pl) |
| [Dog](https://github.com/eyereasoner/eyedual/blob/main/examples/dog.pl) | A compact inheritance chain shows how intermediate concepts appear in a proof. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/dog.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/dog.pl) |
| [Existential rule](https://github.com/eyereasoner/eyedual/blob/main/examples/existential-rule.pl) | Structured Herbrand terms carry explicit generated witnesses. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/existential-rule.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/existential-rule.pl) |
| [Good cobbler](https://github.com/eyereasoner/eyedual/blob/main/examples/good-cobbler.pl) | Multiple premises combine into a conclusion without hidden mutation or control state. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/good-cobbler.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/good-cobbler.pl) |
| [Herbrand Semantics](https://github.com/eyereasoner/eyedual/blob/main/examples/herbrand-semantics.pl) | Herbrand terms denote themselves: distinct names and constructor applications remain distinct without extra unique-name or free-constructor axioms. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/herbrand-semantics.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/herbrand-semantics.pl) |
...
| [Skolem Functions](https://github.com/eyereasoner/eyedual/blob/main/examples/skolem-functions.pl) | Skolem functional terms in rule heads. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/skolem-functions.pl) |
| [Socket Age](https://github.com/eyereasoner/eyedual/blob/main/examples/socket-age.pl) | Socket and plug declarations for an age-reasoning module with registry, policy, and clock providers. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/socket-age.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/socket-age.pl) |
| [Socket Family](https://github.com/eyereasoner/eyedual/blob/main/examples/socket-family.pl) | Socket and plug declarations for a family-reasoning module that expects a `parent/2` provider. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/socket-family.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/socket-family.pl) |
| [Socrates](https://github.com/eyereasoner/eyedual/blob/main/examples/socrates.pl) | A fact and one rule turn the classical syllogism into a ground derivation. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/socrates.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/socrates.pl) |
...
| [Witch](https://github.com/eyereasoner/eyedual/blob/main/examples/witch.pl) | Burn the witch, adapted from Eyeling's examples/witch.n3. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/witch.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/witch.pl) |
...
| Program | What to notice | Checked companions |
| --- | --- | --- |
| [Chart parser](https://github.com/eyereasoner/eyedual/blob/main/examples/chart-parser.pl) | A finite chart represents shared parsing subproblems and recursive grammatical structure. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/chart-parser.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/chart-parser.pl) |
| [Cyclic path](https://github.com/eyereasoner/eyedual/blob/main/examples/cyclic-path.pl) | A deliberately cyclic graph exposes repeated calls and the need for disciplined recursion. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/cyclic-path.pl) |
| [Deep taxonomy: 10](https://github.com/eyereasoner/eyedual/blob/main/examples/deep-taxonomy-10.pl) | A small generated hierarchy is readable by hand and establishes the benchmark shape. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/deep-taxonomy-10.pl) |
| [Deep Taxonomy 100](https://github.com/eyereasoner/eyedual/blob/main/examples/deep-taxonomy-100.pl) | A 100-step taxonomy chain that exercises deep recursive closure and side-label derivation. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/deep-taxonomy-100.pl) |
| [Deep taxonomy: 1,000](https://github.com/eyereasoner/eyedual/blob/main/examples/deep-taxonomy-1000.pl) | The same logical theory tests indexing and recursive closure at a realistic depth. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/deep-taxonomy-1000.pl) |
| [Deep Taxonomy 10000](https://github.com/eyereasoner/eyedual/blob/main/examples/deep-taxonomy-10000.pl) | A 10,000-step taxonomy chain used as a large-depth tabling and closure stress test. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/deep-taxonomy-10000.pl) |
| [Deep taxonomy: 100,000](https://github.com/eyereasoner/eyedual/blob/main/examples/deep-taxonomy-100000.pl) | A stress case separates semantic simplicity from implementation scale. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/deep-taxonomy-100000.pl) |
| [Family cousins](https://github.com/eyereasoner/eyedual/blob/main/examples/family-cousins.pl) | Several relational joins derive kinship beyond a simple transitive closure. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/family-cousins.pl) |
| [Graph reachability](https://github.com/eyereasoner/eyedual/blob/main/examples/graph-reachability.pl) | A visited list bounds cyclic traversal and makes explicit negative test cases finite. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/graph-reachability.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/graph-reachability.pl) |
| [Graph](https://github.com/eyereasoner/eyedual/blob/main/examples/graph.pl) | Productive right-recursive transitive closure over a directed map, contrasted with an under-generating left-recursive formulation. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/graph.pl) |
| [List collection](https://github.com/eyereasoner/eyedual/blob/main/examples/list-collection.pl) | `findall/3`, list construction, and aggregation turn a solution stream into data. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/list-collection.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/list-collection.pl) |
| [Path discovery](https://github.com/eyereasoner/eyedual/blob/main/examples/path-discovery.pl) | Witness paths, not only endpoint pairs, are constructed during a larger graph search. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/path-discovery.pl) |
| [Service Impact](https://github.com/eyereasoner/eyedual/blob/main/examples/service-impact.pl) | Practical cyclic recursion: incident impact analysis for a service dependency graph. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/service-impact.pl) |
...
| --- | --- | --- |
| [Dijkstra Findall Sort](https://github.com/eyereasoner/eyedual/blob/main/examples/dijkstra-findall-sort.pl) | Eyelet-inspired Dijkstra example using findall/3 and sort/2. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/dijkstra-findall-sort.pl) |
| [Dijkstra](https://github.com/eyereasoner/eyedual/blob/main/examples/dijkstra.pl) | Weighted path enumeration adapted from Eyeling dijkstra.n3. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/dijkstra.pl) |
...
| [Enigma1225](https://github.com/eyereasoner/eyedual/blob/main/examples/enigma1225.pl) | New Scientist Enigma 1225, retaining the best board in one pass with `aggregate_max/5`. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/enigma1225.pl) |
| [Eulerian path](https://github.com/eyereasoner/eyedual/blob/main/examples/eulerian-path.pl) | The state tracks remaining edges rather than merely visited vertices. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/eulerian-path.pl) |
...
| [Job-shop scheduling](https://github.com/eyereasoner/eyedual/blob/main/examples/job-shop-scheduling.pl) | Resource and precedence constraints interact in a larger finite schedule space. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/job-shop-scheduling.pl) |
| [Knapsack optimization](https://github.com/eyereasoner/eyedual/blob/main/examples/knapsack-optimization.pl) | Candidate subsets become feasible solutions, then aggregation selects a best value. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/knapsack-optimization.pl) |
| [Map Four Color Search](https://github.com/eyereasoner/eyedual/blob/main/examples/map-four-color-search.pl) | Four-colour search for the European Union neighbour graph. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/map-four-color-search.pl) |
| [Markov Logic Network](https://github.com/eyereasoner/eyedual/blob/main/examples/markov-logic-network.pl) | Markov Logic Network style scoring over a tiny finite domain. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/markov-logic-network.pl) |
| [Matrix Chain Order](https://github.com/eyereasoner/eyedual/blob/main/examples/matrix-chain-order.pl) | Matrix-chain multiplication order by automatically tabled interval dynamic programming. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/matrix-chain-order.pl) |
...
| [Stable marriage](https://github.com/eyereasoner/eyedual/blob/main/examples/stable-marriage.pl) | Preference data, matching generation, and the absence of blocking pairs define stability. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/stable-marriage.pl) |
| [Sudoku 9×9](https://github.com/eyereasoner/eyedual/blob/main/examples/sudoku.pl) | Minimum-remaining-values search fills a standard grid while enforcing row, column, and 3×3 block constraints. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/sudoku.pl) |
| [Weighted interval scheduling](https://github.com/eyereasoner/eyedual/blob/main/examples/weighted-interval-scheduling.pl) | Compatibility constraints and an ordered objective select a maximum-value schedule. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/weighted-interval-scheduling.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/weighted-interval-scheduling.pl) |
...
| --- | --- | --- |
| [Allen Interval Calculus](https://github.com/eyereasoner/eyedual/blob/main/examples/allen-interval-calculus.pl) | Allen interval relations over integer time offsets, with interval records kept as scoped data. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/allen-interval-calculus.pl) |
| [Blocks world](https://github.com/eyereasoner/eyedual/blob/main/examples/blocks-world-planning.pl) | Symbolic actions transform a compact arrangement of blocks. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/blocks-world-planning.pl) |
| [Critical-path schedule](https://github.com/eyereasoner/eyedual/blob/main/examples/critical-path-schedule.pl) | Dependency closure and duration arithmetic derive project timing. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/critical-path-schedule.pl) |
| [Dijkstra Risk Path](https://github.com/eyereasoner/eyedual/blob/main/examples/dijkstra-risk-path.pl) | Risk-adjusted route selection that combines delivery cost, accumulated risk, path length, and a trust gate. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/dijkstra-risk-path.pl) |
| [Dining Philosophers](https://github.com/eyereasoner/eyedual/blob/main/examples/dining-philosophers.pl) | Chandy-Misra dining philosophers trace adapted from Eyeling dining-philosophers.n3. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/dining-philosophers.pl) |
| [Drone corridor planner](https://github.com/eyereasoner/eyedual/blob/main/examples/drone-corridor-planner.pl) | Route feasibility combines graph structure with domain restrictions. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/drone-corridor-planner.pl) |
| [GPS](https://github.com/eyereasoner/eyedual/blob/main/examples/gps.pl) | Route planning over scoped map data, accumulating actions, duration, cost, belief, and comfort. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/gps.pl) |
| [Gray Code Counter](https://github.com/eyereasoner/eyedual/blob/main/examples/gray-code-counter.pl) | Gray-code counter adapted from Eyeling gray-code-counter.n3. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/gray-code-counter.pl) |
| [Hanoi](https://github.com/eyereasoner/eyedual/blob/main/examples/hanoi.pl) | A recursive plan mirrors the inductive structure of moving a tower. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/hanoi.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/hanoi.pl) |
| [Lee routing](https://github.com/eyereasoner/eyedual/blob/main/examples/lee.pl) | Breadth-first wave expansion reaches a destination on a grid, then reconstructs a path around rectangular obstacles using the standard list relations. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/lee.pl) |
| [Microgrid dispatch](https://github.com/eyereasoner/eyedual/blob/main/examples/microgrid-dispatch.pl) | Candidate operating decisions are checked against supply, demand, and engineering limits. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/microgrid-dispatch.pl) |
| [Missionaries and cannibals](https://github.com/eyereasoner/eyedual/blob/main/examples/missionaries-cannibals.pl) | Numeric state constraints must hold on both banks after every crossing. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/missionaries-cannibals.pl) |
| [Monkey and bananas](https://github.com/eyereasoner/eyedual/blob/main/examples/monkey-bananas.pl) | Actions change location, support, and possession facts until the goal becomes true. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/monkey-bananas.pl) |
| [Route planning](https://github.com/eyereasoner/eyedual/blob/main/examples/route-planning.pl) | Weighted edges construct candidate routes and expose the chosen path as a witness. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/route-planning.pl) |
| [Wolf, goat, and cabbage](https://github.com/eyereasoner/eyedual/blob/main/examples/wolf-goat-cabbage.pl) | Safety invariants reject river-bank states before they enter a valid plan. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/wolf-goat-cabbage.pl) |
...
| --- | --- | --- |
| [Ackermann](https://github.com/eyereasoner/eyedual/blob/main/examples/ackermann.pl) | Ackermann-style fast-growing recursion benchmark adapted from Eyeling `ackermann.n3`. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/ackermann.pl) |
| [Binomial Vandermonde](https://github.com/eyereasoner/eyedual/blob/main/examples/binomial-vandermonde.pl) | Two finite sums compute the sides of Vandermonde's identity. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/binomial-vandermonde.pl) |
| [Catalan convolution](https://github.com/eyereasoner/eyedual/blob/main/examples/catalan-convolution.pl) | A classic convolution identity is evaluated over a bounded range. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/catalan-convolution.pl) |
| [Collatz 1000](https://github.com/eyereasoner/eyedual/blob/main/examples/collatz-1000.pl) | Collatz conjecture suite translated from Eyeling's examples/collatz-1000.n3. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/collatz-1000.pl) |
| [Complex](https://github.com/eyereasoner/eyedual/blob/main/examples/complex.pl) | Complex numbers, adapted from Eyeling complex.n3. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/complex.pl) |
| [Composition Of Injective Functions Is Injective](https://github.com/eyereasoner/eyedual/blob/main/examples/composition-of-injective-functions-is-injective.pl) | Composition of injective functions is injective, adapted from Eyeling's examples/composition-of-injective-functions-is-injective.n3. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/composition-of-injective-functions-is-injective.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/composition-of-injective-functions-is-injective.pl) |
| [Continued Fraction Sqrt2](https://github.com/eyereasoner/eyedual/blob/main/examples/continued-fraction-sqrt2.pl) | Convergents of sqrt(2) by automatically tabled recurrence. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/continued-fraction-sqrt2.pl) |
| [D3 group](https://github.com/eyereasoner/eyedual/blob/main/examples/d3-group.pl) | A finite Cayley table, inverses, and subgroup closure make group laws executable. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/d3-group.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/d3-group.pl) |
| [Diamond Property](https://github.com/eyereasoner/eyedual/blob/main/examples/diamond-property.pl) | Diamond property, adapted from Eyelet's input/diamond-property.pl. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/diamond-property.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/diamond-property.pl) |
| [Easter Computus](https://github.com/eyereasoner/eyedual/blob/main/examples/easter-computus.pl) | Gregorian Easter computus adapted from Eyeling's easter.n3. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/easter-computus.pl) |
| [Equivalence Classes Overlap Implies Same Class](https://github.com/eyereasoner/eyedual/blob/main/examples/equivalence-classes-overlap-implies-same-class.pl) | Equivalence-class overlap example adapted from Eyeling. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/equivalence-classes-overlap-implies-same-class.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/equivalence-classes-overlap-implies-same-class.pl) |
| [Fast exponentiation](https://github.com/eyereasoner/eyedual/blob/main/examples/fastpow.pl) | Algebraic decomposition by parity changes a linear recurrence into logarithmic-depth recursion. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/fastpow.pl) |
| [Fibonacci](https://github.com/eyereasoner/eyedual/blob/main/examples/fibonacci.pl) | A recurrence becomes an executable relation with a visibly decreasing argument. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/fibonacci.pl) |
| [Fundamental theorem of arithmetic](https://github.com/eyereasoner/eyedual/blob/main/examples/fundamental-theorem-arithmetic.pl) | Two factorization strategies construct normalized prime-factor witnesses and check reconstruction. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/fundamental-theorem-arithmetic.pl) |
| [Goldbach](https://github.com/eyereasoner/eyedual/blob/main/examples/goldbach.pl) | Bounded search checks Goldbach decompositions for powers of two; the default native `smallest_divisor_from/3` accelerator makes the range through `2^35` practical. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/goldbach.pl) |
| [Greatest lower bound uniqueness](https://github.com/eyereasoner/eyedual/blob/main/examples/greatest-lower-bound-uniqueness.pl) | Order-theoretic definitions support a uniqueness argument. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/greatest-lower-bound-uniqueness.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/greatest-lower-bound-uniqueness.pl) |
| [Group inverse uniqueness](https://github.com/eyereasoner/eyedual/blob/main/examples/group-inverse-uniqueness.pl) | A short derivation exposes the algebraic premises needed for uniqueness. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/group-inverse-uniqueness.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/group-inverse-uniqueness.pl) |
| [Heron Theorem](https://github.com/eyereasoner/eyedual/blob/main/examples/heron-theorem.pl) | Heron's theorem: area = sqrt(s(s-a)(s-b)(s-c)). | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/heron-theorem.pl) |
| [Integer partitions](https://github.com/eyereasoner/eyedual/blob/main/examples/integer-partitions.pl) | Recursive generation constructs unordered additive decompositions without permutation duplicates. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/integer-partitions.pl) |
| [Law Of Cosines](https://github.com/eyereasoner/eyedual/blob/main/examples/law-of-cosines.pl) | Law of cosines: c^2 = a^2 + b^2 - 2ab cos(C). | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/law-of-cosines.pl) |
| [Matrix noncommutativity](https://github.com/eyereasoner/eyedual/blob/main/examples/matrix-noncommutativity.pl) | Two concrete products provide a counterexample to universal commutativity. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/matrix-noncommutativity.pl) |
| [Modular exponentiation](https://github.com/eyereasoner/eyedual/blob/main/examples/modular-exponentiation.pl) | Intermediate reduction preserves the residue while controlling numeric growth. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/modular-exponentiation.pl) |
| [Newton Raphson](https://github.com/eyereasoner/eyedual/blob/main/examples/newton-raphson.pl) | Newton-Raphson root finding, adapted from Eyelet input/newton-raphson.pl. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/newton-raphson.pl) |
| [Peano arithmetic](https://github.com/eyereasoner/eyedual/blob/main/examples/peano-arithmetic.pl) | Explicit natural-number terms support arithmetic relations and structural recursion. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/peano-arithmetic.pl) |
| [Peano calculus](https://github.com/eyereasoner/eyedual/blob/main/examples/peano-calculus.pl) | Addition, multiplication, and factorial follow the constructors `z` and `s/1`. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/peano-calculus.pl) |
| [Peasant](https://github.com/eyereasoner/eyedual/blob/main/examples/peasant.pl) | Peasant multiplication and exponentiation cases, adapted from Eyelet. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/peasant.pl) |
| [Pell equation](https://github.com/eyereasoner/eyedual/blob/main/examples/pell-equation.pl) | Bounded generation searches for integer witnesses to a Diophantine equation. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/pell-equation.pl) |
| [Pi](https://github.com/eyereasoner/eyedual/blob/main/examples/pi.pl) | The Nilakantha series is a deterministic numeric recurrence; EyeDual recognizes its accumulator shape and executes 10,000 terms without tabling or heap growth. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/pi.pl) |
| [Prime range](https://github.com/eyereasoner/eyedual/blob/main/examples/prime-range.pl) | Bounded integer generation and divisor tests enumerate primes over an explicit finite interval. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/prime-range.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/prime-range.pl) |
| [Quadratic Formula](https://github.com/eyereasoner/eyedual/blob/main/examples/quadratic-formula.pl) | Quadratic formula over sample equations. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/quadratic-formula.pl) |
| [Riemann Hypothesis](https://github.com/eyereasoner/eyedual/blob/main/examples/riemann-hypothesis.pl) | A deliberately finite audit of catalogued non-trivial zeros, illustrating the boundary between evidence and universal proof. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/riemann-hypothesis.pl) |
| [Shoelace Polygon Area](https://github.com/eyereasoner/eyedual/blob/main/examples/shoelace-polygon-area.pl) | Polygon area by the shoelace formula. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/shoelace-polygon-area.pl) |
| [Sieve](https://github.com/eyereasoner/eyedual/blob/main/examples/sieve.pl) | List filtering presents a different operational route to finite prime generation. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/sieve.pl) |
| [Stirling and Bell numbers](https://github.com/eyereasoner/eyedual/blob/main/examples/stirling-bell-numbers.pl) | Inclusion–exclusion and recurrence count set partitions in two related ways. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/stirling-bell-numbers.pl) |
| [Takeuchi](https://github.com/eyereasoner/eyedual/blob/main/examples/takeuchi.pl) | The Takeuchi function as a demanding nested-recursion benchmark. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/takeuchi.pl) |
| [Totient summatory function](https://github.com/eyereasoner/eyedual/blob/main/examples/totient-summatory.pl) | Divisibility, coprimality, counting, and summation compose over finite domains. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/totient-summatory.pl) |
...
discipline is to keep object language and EyeDual metalanguage distinct.

| Program | What the terms represent | Checked answer |
| --- | --- | --- |
| [SAT solver: CDCL](https://github.com/eyereasoner/eyedual/blob/main/examples/cdcl-sat-solver.pl) | The example extends the SAT vocabulary toward conflicts and learned information. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/cdcl-sat-solver.pl) |
| [Chart parser](https://github.com/eyereasoner/eyedual/blob/main/examples/chart-parser.pl) | Shared chart items prevent grammatical subproblems from being rediscovered independently. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/chart-parser.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/chart-parser.pl) |
| [Context Schema Audit](https://github.com/eyereasoner/eyedual/blob/main/examples/context-schema-audit.pl) | Schema auditing for heterogeneous context terms by decomposing members with `=../2` and checking predicate arity. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/context-schema-audit.pl) |
| [Derived Backward Rule](https://github.com/eyereasoner/eyedual/blob/main/examples/derived-backward-rule.pl) | Derived backward rule example adapted from Eyeling derived-backward-rule.n3. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/derived-backward-rule.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/derived-backward-rule.pl) |
| [Equality saturation](https://github.com/eyereasoner/eyedual/blob/main/examples/equality-saturation.pl) | Repeated rewrite closure explores equivalent symbolic forms to a fixed point. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/equality-saturation.pl) |
| [Expression evaluator](https://github.com/eyereasoner/eyedual/blob/main/examples/expression-eval.pl) | Arithmetic expression trees are interpreted under an explicit environment. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/expression-eval.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/expression-eval.pl) |
| [Fast Fourier Transform](https://github.com/eyereasoner/eyedual/blob/main/examples/fast-fourier-transform.pl) | Recursive evaluation builds a shared expression tree and treats graphic operators such as `+` and `*` as data atoms. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/fast-fourier-transform.pl) |
| [Intuitionistic Logic Kripke](https://github.com/eyereasoner/eyedual/blob/main/examples/intuitionistic-logic-kripke.pl) | Intuitionistic logic emulation with a finite Kripke model. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/intuitionistic-logic-kripke.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/intuitionistic-logic-kripke.pl) |
| [Knuth–Bendix completion](https://github.com/eyereasoner/eyedual/blob/main/examples/knuth-bendix-completion.pl) | Oriented equations and critical interactions seek a more canonical rewrite system. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/knuth-bendix-completion.pl) |
| [Language](https://github.com/eyereasoner/eyedual/blob/main/examples/language.pl) | A small grammar recognizes a finite relational language. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/language.pl) |
| [Linear Logic Resources](https://github.com/eyereasoner/eyedual/blob/main/examples/linear-logic-resources.pl) | Linear logic emulation with explicit consumable resources. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/linear-logic-resources.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/linear-logic-resources.pl) |
| [Modal Logic Kripke](https://github.com/eyereasoner/eyedual/blob/main/examples/modal-logic-kripke.pl) | Modal logic emulation with a finite Kripke frame. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/modal-logic-kripke.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/modal-logic-kripke.pl) |
| [Partial evaluator](https://github.com/eyereasoner/eyedual/blob/main/examples/partial-evaluator.pl) | Known inputs specialize an expression or program while unknown parts remain symbolic. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/partial-evaluator.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/partial-evaluator.pl) |
| [Polynomial](https://github.com/eyereasoner/eyedual/blob/main/examples/polynomial.pl) | Structured coefficients and powers support symbolic polynomial operations. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/polynomial.pl) |
| [Proof Contrapositive](https://github.com/eyereasoner/eyedual/blob/main/examples/proof-contrapositive.pl) | Proof by contrapositive example adapted from Eyelet input/proof-by-contrapositive.pl. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/proof-contrapositive.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/proof-contrapositive.pl) |
| [Quine–McCluskey](https://github.com/eyereasoner/eyedual/blob/main/examples/quine-mccluskey.pl) | Boolean minimization with the Quine–McCluskey method, including essential implicants and deterministic cover selection. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/quine-mccluskey.pl) |
| [SAT solver: DPLL](https://github.com/eyereasoner/eyedual/blob/main/examples/sat-solver-dpll.pl) | Formula representation, assignment, simplification, and branching form a compact solver. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/sat-solver-dpll.pl) |
| [Symbolic derivative](https://github.com/eyereasoner/eyedual/blob/main/examples/symbolic-derivative.pl) | Differentiation rules transform expression trees without evaluating them numerically; the proof golden exposes the recursive construction. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/symbolic-derivative.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/symbolic-derivative.pl) |
| [Turing machine](https://github.com/eyereasoner/eyedual/blob/main/examples/turing.pl) | Machine configuration terms and transition rules expose a classical computation model. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/turing.pl) |
...
nested terms is automatically asserted as an EyeDual goal.
...
| --- | --- | --- |
| [Abstract interpretation](https://github.com/eyereasoner/eyedual/blob/main/examples/abstract-interpretation.pl) | A finite sign domain conservatively approximates many concrete executions. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/abstract-interpretation.pl) |
| [Cache performance](https://github.com/eyereasoner/eyedual/blob/main/examples/cache-performance.pl) | Configuration and workload facts derive performance classifications and reasons. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/cache-performance.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/cache-performance.pl) |
| [Canary release](https://github.com/eyereasoner/eyedual/blob/main/examples/canary-release.pl) | Observations and thresholds support a deployment decision with auditable evidence. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/canary-release.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/canary-release.pl) |
| [Network SLA](https://github.com/eyereasoner/eyedual/blob/main/examples/network-sla.pl) | Technology example: network path SLA check. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/network-sla.pl) |
| [Observability log correlation](https://github.com/eyereasoner/eyedual/blob/main/examples/observability-log-correlation.pl) | Structured log events join across identifiers and time-related facts. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/observability-log-correlation.pl) |
| [Pointer analysis](https://github.com/eyereasoner/eyedual/blob/main/examples/pointer-analysis.pl) | Allocation and assignment constraints derive a points-to relation by closure. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/pointer-analysis.pl) |
| [Register allocation](https://github.com/eyereasoner/eyedual/blob/main/examples/register-allocation.pl) | Liveness interference becomes a finite coloring problem. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/register-allocation.pl) |
| [Relational Cube Lookup](https://github.com/eyereasoner/eyedual/blob/main/examples/relational-cube-lookup.pl) | Performance example: repeated multi-key relational lookups. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/relational-cube-lookup.pl) |
| [Security incident correlation](https://github.com/eyereasoner/eyedual/blob/main/examples/security-incident-correlation.pl) | Distributed observations combine into incident conclusions. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/security-incident-correlation.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/security-incident-correlation.pl) |
| [Truth-maintenance system](https://github.com/eyereasoner/eyedual/blob/main/examples/truth-maintenance-system.pl) | Justifications remain explicit when conclusions depend on defeasible information. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/truth-maintenance-system.pl) |
| [Type inference](https://github.com/eyereasoner/eyedual/blob/main/examples/type-inference.pl) | Structural unification solves type constraints for a tiny expression language. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/type-inference.pl) |
| [Vulnerability Impact](https://github.com/eyereasoner/eyedual/blob/main/examples/vulnerability-impact.pl) | Vulnerability impact analysis over a transitive dependency graph. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/vulnerability-impact.pl) |
...
| --- | --- | --- |
| [Access control policy](https://github.com/eyereasoner/eyedual/blob/main/examples/access-control-policy.pl) | Attribute and policy facts derive permit status and reasons. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/access-control-policy.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/access-control-policy.pl) |
...
| [Deontic Logic](https://github.com/eyereasoner/eyedual/blob/main/examples/deontic-logic.pl) | Deontic logic: obligations, prohibitions, compensations, and violations. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/deontic-logic.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/deontic-logic.pl) |
...
| [Illegitimate Reasoning](https://github.com/eyereasoner/eyedual/blob/main/examples/illegitimate-reasoning.pl) | Illegitimate reasoning detector. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/illegitimate-reasoning.pl) |
| [Integrity check](https://github.com/eyereasoner/eyedual/blob/main/examples/integrity-check.pl) | An explicit invalid-state relation reports contradictory input and a diagnostic status. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/integrity-check.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/integrity-check.pl) |
| [Nixon Diamond](https://github.com/eyereasoner/eyedual/blob/main/examples/nixon-diamond.pl) | Nixon diamond: two independent defaults support incompatible conclusions. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/nixon-diamond.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/nixon-diamond.pl) |
...
| [ODRL policy from Turtle](https://github.com/eyereasoner/eyedual/blob/main/examples/odrl-policy-from-turtle.pl) | An ODRL permission, target, action, and purpose constraint are loaded from [Turtle](https://github.com/eyereasoner/eyedual/blob/main/examples/input/odrl-policy.ttl) and reduced to one deterministic permit decision by a [small rule file](https://github.com/eyereasoner/eyedual/blob/main/examples/input/odrl-policy-rules.pl). | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/odrl-policy-from-turtle.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/odrl-policy-from-turtle.pl) |
...
| --- | --- | --- |
| [Bayes Diagnosis](https://github.com/eyereasoner/eyedual/blob/main/examples/bayes-diagnosis.pl) | Bayesian diagnosis adapted from Eyeling bayes-diagnosis.n3. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/bayes-diagnosis.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/bayes-diagnosis.pl) |
| [Bayes Therapy](https://github.com/eyereasoner/eyedual/blob/main/examples/bayes-therapy.pl) | Memoize shared inference layers: the score vector, disease likelihood tails, and expected therapy success are reused by several report relations. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/bayes-therapy.pl) |
| [Beam deflection](https://github.com/eyereasoner/eyedual/blob/main/examples/beam-deflection.pl) | A mechanics equation combines load, geometry, and material parameters. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/beam-deflection.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/beam-deflection.pl) |
| [BMI](https://github.com/eyereasoner/eyedual/blob/main/examples/bmi.pl) | Metric and US-unit normalization, BMI classification, healthy-weight bands, and audit checks. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/bmi.pl) |
| [Braking Safety Worlds](https://github.com/eyereasoner/eyedual/blob/main/examples/braking-safety-worlds.pl) | EYE reasoning-inspired example: braking safety in alternative worlds. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/braking-safety-worlds.pl) |
| [Buck converter design](https://github.com/eyereasoner/eyedual/blob/main/examples/buck-converter-design.pl) | Electrical design candidates are checked against component and performance constraints. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/buck-converter-design.pl) |
| [Competitive enzyme kinetics](https://github.com/eyereasoner/eyedual/blob/main/examples/competitive-enzyme-kinetics.pl) | A biochemical rate law becomes a numeric relational model. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/competitive-enzyme-kinetics.pl) |
| [Control system](https://github.com/eyereasoner/eyedual/blob/main/examples/control-system.pl) | System parameters derive stability- and response-related quantities. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/control-system.pl) |
| [Dairy energy balance](https://github.com/eyereasoner/eyedual/blob/main/examples/dairy-energy-balance.pl) | Intake and expenditure quantities are combined in an agricultural model. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/dairy-energy-balance.pl) |
| [Electrical RC filter](https://github.com/eyereasoner/eyedual/blob/main/examples/electrical-rc-filter.pl) | Component values derive circuit behavior under an explicit formula. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/electrical-rc-filter.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/electrical-rc-filter.pl) |
| [Epidemic policy](https://github.com/eyereasoner/eyedual/blob/main/examples/epidemic-policy.pl) | Observations and thresholds connect a simple epidemic model to policy conclusions. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/epidemic-policy.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/epidemic-policy.pl) |
| [EV Range Worlds](https://github.com/eyereasoner/eyedual/blob/main/examples/ev-range-worlds.pl) | EYE-inspired electric-vehicle range worlds. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/ev-range-worlds.pl) |
| [Exoplanet Validation Worlds](https://github.com/eyereasoner/eyedual/blob/main/examples/exoplanet-validation-worlds.pl) | EYE reasoning-inspired example: exoplanet candidate validation worlds. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/exoplanet-validation-worlds.pl) |
| [FFT-8 Numeric](https://github.com/eyereasoner/eyedual/blob/main/examples/fft8-numeric.pl) | An eight-point radix-2 FFT over explicit complex pairs, showing butterflies, twiddle factors, and selected bins. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/fft8-numeric.pl) |
| [Field nitrogen balance](https://github.com/eyereasoner/eyedual/blob/main/examples/field-nitrogen-balance.pl) | Inputs, removal, and losses form a conservation-style accounting relation. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/field-nitrogen-balance.pl) |
| [GD Step Certified](https://github.com/eyereasoner/eyedual/blob/main/examples/gd-step-certified.pl) | A proof-friendly certified gradient-descent step with memoized interval bounds and explicit acceptance evidence. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/gd-step-certified.pl) |
| [Hamming Code](https://github.com/eyereasoner/eyedual/blob/main/examples/hamming-code.pl) | Technology example: Hamming(7,4) single-bit error correction. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/hamming-code.pl) |
| [Heat Loss](https://github.com/eyereasoner/eyedual/blob/main/examples/heat-loss.pl) | Engineering example: one-dimensional conductive heat loss through a wall. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/heat-loss.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/heat-loss.pl) |
| [Ideal Gas Law](https://github.com/eyereasoner/eyedual/blob/main/examples/ideal-gas-law.pl) | Science example: ideal gas law. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/ideal-gas-law.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/ideal-gas-law.pl) |
| [Least-squares regression](https://github.com/eyereasoner/eyedual/blob/main/examples/least-squares-regression.pl) | Finite observations are summarized into a fitted linear model. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/least-squares-regression.pl) |
| [Orbital transfer design](https://github.com/eyereasoner/eyedual/blob/main/examples/orbital-transfer-design.pl) | Candidate orbital parameters are evaluated against transfer equations. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/orbital-transfer-design.pl) |
| [Pendulum Period](https://github.com/eyereasoner/eyedual/blob/main/examples/pendulum-period.pl) | Science example: simple pendulum period. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/pendulum-period.pl) |
| [Radioactive Decay](https://github.com/eyereasoner/eyedual/blob/main/examples/radioactive-decay.pl) | Science example: radioactive decay. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/radioactive-decay.pl) |
| [Spacecraft battery diagnosis](https://github.com/eyereasoner/eyedual/blob/main/examples/spacecraft-battery-diagnosis.pl) | Telemetry, `P = I²R`, limits, and redundant sensing support diagnosis and action. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/spacecraft-battery-diagnosis.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/spacecraft-battery-diagnosis.pl) |
| [Statistics summary](https://github.com/eyereasoner/eyedual/blob/main/examples/statistics-summary.pl) | Aggregates compute descriptive statistics over a finite list. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/statistics-summary.pl) |
| [Superdense Coding](https://github.com/eyereasoner/eyedual/blob/main/examples/superdense-coding.pl) | Superdense coding using discrete quantum computing, adapted from Eyelet's input/superdense-coding.pl. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/superdense-coding.pl) |
| [Vector Similarity](https://github.com/eyereasoner/eyedual/blob/main/examples/vector-similarity.pl) | Vector dot product, Euclidean norm, and cosine similarity. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/vector-similarity.pl) |
...
the source data, generated EyeDual facts, rules, answers, and serialized RDF as
one adapter pipeline.

| Program | RDF feature | Checked answer |
| --- | --- | --- |
| [Aliases and namespaces](https://github.com/eyereasoner/eyedual/blob/main/examples/aliases-and-namespaces.pl) | Explicit name relations avoid adding hidden namespace semantics to the core. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/aliases-and-namespaces.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/aliases-and-namespaces.pl) |
| [Alignment Demo](https://github.com/eyereasoner/eyedual/blob/main/examples/alignment-demo.pl) | Broader and narrower concept alignments, their transitive closure, and roll-up queries. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/alignment-demo.pl) |
| [Context Association](https://github.com/eyereasoner/eyedual/blob/main/examples/context-association.pl) | Context association adapted from Eyeling context-association.n3. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/context-association.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/context-association.pl) |
...
| [Directional language](https://github.com/eyereasoner/eyedual/blob/main/examples/rdf12-directional-language.pl) | Language direction remains explicit in the lossless literal encoding. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/rdf12-directional-language.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/rdf12-directional-language.pl) |
...
| [TriG graph join](https://github.com/eyereasoner/eyedual/blob/main/examples/rdf12-trig-graph-join.pl) | Rules join facts while retaining their graph-sensitive representation. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/rdf12-trig-graph-join.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/rdf12-trig-graph-join.pl) |
...
| [TriG triple term](https://github.com/eyereasoner/eyedual/blob/main/examples/rdf12-trig-triple-term.pl) | RDF 1.2 triple terms and dataset graph structure appear together. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/rdf12-trig-triple-term.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/rdf12-trig-triple-term.pl) |
...
| [Web names](https://github.com/eyereasoner/eyedual/blob/main/examples/web-names.pl) | Quoted web identifiers remain atom constants in ordinary Prolog terms. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/web-names.pl) · [proof](https://github.com/eyereasoner/eyedual/blob/main/examples/proof/web-names.pl) |

The original RDF fixtures and adapter rules are available in
[examples/input](https://github.com/eyereasoner/eyedual/tree/main/examples/input/). Chapter 15 explains why the conversion is
...
| --- | --- | --- |
| [AuroraCare](https://github.com/eyereasoner/eyedual/blob/main/examples/auroracare.pl) | A large healthcare-oriented knowledge theory combines many domain concepts and decisions. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/auroracare.pl) |
| [Basic monadic](https://github.com/eyereasoner/eyedual/blob/main/examples/basic-monadic.pl) | A large generated symbolic theory stresses parsing, terms, and relational execution. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/basic-monadic.pl) |
| [Delfour](https://github.com/eyereasoner/eyedual/blob/main/examples/delfour.pl) | Delfour insight-economy case adapted from Eyeling delfour.n3. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/delfour.pl) |
| [Flandor](https://github.com/eyereasoner/eyedual/blob/main/examples/flandor.pl) | A broad rule set provides practice navigating a less tutorial-shaped theory. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/flandor.pl) |
| [Knowledge-engineering alignment flow](https://github.com/eyereasoner/eyedual/blob/main/examples/knowledge-engineering-alignment-flow.pl) | Source concepts, mappings, validation, and derived alignment are kept in explicit layers. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/knowledge-engineering-alignment-flow.pl) |
| [LLDM](https://github.com/eyereasoner/eyedual/blob/main/examples/lldm.pl) | A larger logical model demonstrates layered derivation over substantial source data. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/lldm.pl) |
| [Manufacturing quality control](https://github.com/eyereasoner/eyedual/blob/main/examples/manufacturing-quality-control.pl) | Measurements, limits, classifications, and actions form an auditable industrial decision. | [answers](https://github.com/eyereasoner/eyedual/blob/main/examples/output/manufacturing-quality-control.pl) |
...
This book is the single reference for the EyeDual implementation. Chapters 38–40
...
EyeDual executes a documented and tested **ISO compatibility profile** based on
...
The executable examples are EyeDual-profile programs using host-supplied goals,
strings, explicit integrity relations, automatic tabling, and the EyeDual library.
...
The EyeDual corpus verifies this documented profile; it is not an independent
certification that every conforming Prolog text will run unchanged.

### Security and resource use

EyeDual has no general host-call primitive, yet an untrusted theory is still
...
The book is self-contained as an EyeDual guide. These sources provide historical
and technical background for the ideas that EyeDual adapts. They describe larger
languages and theories, so they should not be read as additional EyeDual
specifications.
...
- ISO/IEC,
  ...
  Chapter 38 defines the precise EyeDual compatibility profile against this
...
- Yoshihiko Futamura,
  ...
  as historical context for specialization, not as an EyeDual implementation
  claim.
...
- Weidong Chen and David S. Warren,
  ...
  evaluation. EyeDual's automatic positive tabling is smaller in scope, but the
...
- Dörthe Arndt and Stephan Mennicke,
  ...
  languages and existential-rule reasoning. EyeDual deliberately implements a
...
- Leon Sterling and Ehud Shapiro,
  ...
  EyeDual differs substantially from full Prolog, so the material here develops
  those themes only through EyeDual's explicit, supported relations.

The aim of EyeDual is not to make every difficult problem easy. It is to keep the
...
This glossary fixes the book's vocabulary. Definitions describe EyeDual unless a
...
search. EyeDual suppresses duplicate printed answers and source facts already
...
sufficiently scoped claim as evidence for its absence. EyeDual's `\+/1` performs
...
property. EyeDual does not provide a general persistent constraint store.
...
**Ground.** Containing no variables. EyeDual prints only ground query answers.
...
term containing that variable. EyeDual performs it consistently for ordinary
...
- query the generated Prolog program with EyeDual directly;
...
**Reflect:** what simplicity does the explicit adapter preserve in the EyeDual
core?
...
`eyedual --goal 'child(X, Y)' program.pl` asks for every ground child–parent pair derivable by the
